...
class Aggregate(nn.Module):
    def __init__(self, len_feature):
        ...
        self.conv_1 = nn.Sequential(
            nn.Conv1d(in_channels=len_feature, out_channels=self.out_len, kernel_size=3,
...
class Model(nn.Module):
    def __init__(self,args, n_features, batch_size,num_segments):
        ...
        if args.model_name == 'ssrl_stage1':
            ...
            for size in self.multi_patch_size:
                Aggregate_patch.append(Aggregate(len_feature=2048))
                patch_to_clip.append(self.build_patch_to_clip(patch_size=size))
                linear_out.append(self.build_linear_out(patch_size=size))

            self.Aggregate_patch = nn.ModuleList(Aggregate_patch)
            self.patch_to_clip = nn.ModuleList(patch_to_clip)
            self.linear_out = nn.ModuleList(linear_out)
            self.Aggregate_clip = Aggregate(len_feature=2048)
               
        self.drop_out = nn.Dropout(0.7)
        self.classfier = Classfier(n_features)
        self.relu = nn.ReLU()
        self.apply(weight_init)

    def build_patch_to_clip(self,dim_in=2048,dim_out=1024,patch_size=35):
        return nn.Sequential(
            nn.Conv2d(in_channels=dim_in, out_channels=dim_out, kernel_size=3, stride=1, padding=1 if patch_size == 23 else 0 ),
            nn.ReLU(),
            nn.BatchNorm2d(dim_out),
        )
    def build_linear_out(self,dim_out=2048,patch_size=35):
        dim_in = {"23":6144,"35":3072,"47":10240}
        return nn.Linear(dim_in[str(patch_size)],dim_out)
    ...
    def forward(self, inputs):
        ...
        if self.model_name == 'ssrl_stage1':
            ...
            for i , size in enumerate(self.multi_patch_size):
                input_patch[i] = self.Aggregate_patch[i](input_patch[i].permute(0,2,1,3).reshape(bs*t,-1,f))
                input_patch[i] = input_patch[i].permute(0,2,1).view(bs*t,f,size//10,size%10)
                input_patch[i] = self.patch_to_clip[i](input_patch[i]).reshape(bs,t,-1)
                input_patch[i] = self.relu(self.linear_out[i](input_patch[i]))
                if size == 47:
                    input_patch[i] = self.drop_out(input_patch[i])     
                
                input_clip = input_clip + input_patch[i].unsqueeze(1)

            out = self.Aggregate_clip(input_clip.reshape(-1,t,f))
            ncrops = 10

        out = self.drop_out(out)
        features = out
        scores = self.classfier(features)
